...
def call_chat(client: OpenAI, model: str, messages: List[Dict[str, str]], temperature: float = 1.3, max_tokens: int = 6144) -> str:
...
def _load_prompt_or_persona(path: str, row_number: int) -> str:
...
def run_persona_translation(
    ...
    questions = read_xlsx_questions(questions_xlsx, 2, 6)

    df_columns = [f"Q{i + 1}" for i in range(24)]
    df = pd.DataFrame(columns=df_columns)

    for window_id in range(1, n_rows + 1):
        prompt = _load_prompt_or_persona(system_prompt_txt, window_id)
        persona = _load_prompt_or_persona(persona_xlsx, window_id)
        messages = [
            {"role": "system", "content": prompt},
            {"role": "system", "content": persona},
            {"role": "user", "content": " ".join(questions)},
        ]
        trim = trim_messages(messages.copy())
        try:
    ...
    if os.path.exists(out_xlsx):
        with pd.ExcelWriter(out_xlsx, mode='a', engine='openpyxl', if_sheet_exists='overlay') as writer:
            sheet_name = 'Sheet1'
