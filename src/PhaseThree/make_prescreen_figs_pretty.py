...
# Font setup (adjust to your environment if needed)
...
def main():
    df = load_data()
    plot_band(df, 0.5, 'prescreen_passrate_band_0.5.png')
    plot_band(df, 1.0, 'prescreen_passrate_band_1.0.png')
    for band in [0.5, 1.0]:
        for m in ['AIPG','IRPG','RDPS']:
            plot_by_method(df, band, m, f'{m}_only_band_{band}.png')
