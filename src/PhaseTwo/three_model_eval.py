...
def main():
    # Default API and basic config (for models other than qwen3)
    # Use environment variables for security
    default_api_key = os.getenv("OPENAI_API_KEY", '')
    default_base_url = os.getenv("OPENAI_BASE_URL", "https://api.gptsapi.net/v1")

    # Specify running only gpt-5-chat model
    models = [
        "gpt-5-chat",
    ]

    # Model level config overrides (no special overrides needed here)
    model_overrides = {}
